...
def draw_main_floor(ax, config: dict[str, Any]) -> None:
    ...
    # Add furniture if present
    furniture = main_config.get("furniture", [])
    draw_furniture_from_data(ax, furniture)

    # Add text annotations if present
    text_annotations = main_config.get("text_annotations", [])
    draw_text_annotations_from_data(ax, text_annotations)

    # Add dimension annotations
    dims = main_config.get("dimensions", {})
    fig_y_max = fig_config.get("y_max", 80)
    fig_x_max = fig_config.get("x_max", 130)
    add_dimension_arrow(
        ax,
        (scale(dims.get("height_arrow_x", -5)), scale(0)),
        (scale(dims.get("height_arrow_x", -5)), scale(fig_y_max - 10)),
        dims.get("height_label", "~70'"),
        offset=scale(-1),
        rotation=90,
    )
    add_dimension_arrow(
        ax,
        (scale(0), scale(dims.get("width_arrow_y", -15))),
        (scale(fig_x_max - 5), scale(dims.get("width_arrow_y", -15))),
        dims.get("width_label", "~125'"),
        offset=scale(-1),
    )

    # Add north arrow if enabled
    if settings.get("show_north_arrow", True):
...
def draw_basement(ax, config: dict[str, Any]) -> None:
    ...
    if get_debug_mode():
        draw_debug_grid(
            ...
            fig_config.get("x_max", 110),
            fig_config.get("y_min", -5),
            fig_config.get("y_max", 70),
            get_grid_spacing(),
        )
    ...
    # ===== Home Theater (only if configured) =====
    theater_config = basement_config.get("theater", {})
    if theater_config:
        theater_room = theater_config.get("room", {})

        # Only draw theater room overlay if room data has actual dimensions
        if theater_room and theater_room.get("width"):
            add_room_simple(
                ax,
                theater_room.get("x", 0),
                theater_room.get("y", 35),
                theater_room.get("width", 30),
                theater_room.get("height", 25),
                theater_room.get("label", "Home Theater"),
                theater_room.get("color", "theater"),
                theater_room.get("label_fontsize", 12),
                theater_room.get("label_color", "white"),
                theater_room.get("fontweight", "bold"),
                theater_room.get("auto_dimension", True),
                theater_room.get("dimension_text", ""),
            )

        # Theater seating - only draw if seating is a dict with actual row/seat data
        seating_config = theater_config.get("seating", {})
        if isinstance(seating_config, dict) and seating_config:
            rows = seating_config.get("rows", 0)
            seats = seating_config.get("seats_per_row", 0)
            if rows > 0 and seats > 0:
                seating = TheaterSeating(
                    start_x=seating_config.get("start_x", 3),
                    start_y=seating_config.get("start_y", 37),
                    rows=rows,
                    seats_per_row=seats,
        ...
        # False wall - only draw if configured with coordinates
        false_wall = theater_config.get("false_wall", {})
        if false_wall and false_wall.get("x1") is not None:
            line = LineAnnotation(
                x1=false_wall.get("x1", 0),
                y1=false_wall.get("y1", 0),
                x2=false_wall.get("x2", 30),
            ...
            if false_wall.get("label"):
                ax.text(
                    scale(false_wall.get("label_x", 15)),
    ...
    # ===== Pool Area (only if configured with actual dimensions) =====
    pool_config = basement_config.get("pool", {})
    if pool_config:
        area = pool_config.get("area", {})
        # Only draw pool if area has a width defined (meaning it's actually configured)
        if area and area.get("width", 0) > 0:
            pool = pool_config.get("pool", pool_config.get("pool_water", {}))
            ...
            pool_data = PoolConfig(
                area_x=area.get("x", 0),
                area_y=area.get("y", 0),
                area_width=area.get("width", 25),
                area_height=area.get("height", 35),
                pool_x=pool.get("x", 5),
                pool_y=pool.get("y", 5),
                pool_width=pool.get("width", 15),
                pool_height=pool.get("height", 25),
                hot_tub_x=hot_tub.get("x", 0),
                hot_tub_y=hot_tub.get("y", 0),
                ...
                hot_tub_radius=hot_tub.get("radius", 0),
                hot_tub_color=hot_tub.get("color", "spa"),
                hot_tub_label=hot_tub.get("label", ""),
                spa_label_x=spa_label.get("x", 0),
                spa_label_y=spa_label.get("y", 0),
            )
    ...
    fig_y_max = fig_config.get("y_max", 70)
    fig_x_max = fig_config.get("x_max", 110)
    add_dimension_arrow(
        ax,
        (scale(dims.get("height_arrow_x", -3)), scale(0)),
        (scale(dims.get("height_arrow_x", -3)), scale(fig_y_max - 10)),
    ...
    add_dimension_arrow(
        ax,
        (scale(0), scale(dims.get("width_arrow_y", -5))),
        (scale(fig_x_max - 10), scale(dims.get("width_arrow_y", -5))),
    ...
    if ceiling_note and ceiling_note.get("text"):
        ax.text(
            scale(ceiling_note.get("x", 50)),
            scale(ceiling_note.get("y", 65)),
            ceiling_note.get("text", ""),
            fontsize=ceiling_note.get("fontsize", 9),
            ha="center",
    ...
    if settings.get("show_north_arrow", True):
        ...
        add_north_arrow(
            ax,
            north_config.get("x", 100),
            north_config.get("y", 62),
